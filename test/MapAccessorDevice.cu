...
{
    map.set_value(1, cuvoxmap::Idx2D{1, 2});
    map.set_value(2, cuvoxmap::Idx2D{2, 3});

    if (!CUSTOM_TEST_KERNEL::KERNEL_TEST(map.get_value(cuvoxmap::Idx2D{1, 2}), 1.0f))
        return;
    if (!CUSTOM_TEST_KERNEL::KERNEL_TEST(map.get_value(cuvoxmap::Idx2D{2, 3}), 2.0f))
        return;
    ...
    if (!CUSTOM_TEST_KERNEL::KERNEL_TEST(map.get_axis_size()[1], 20u))
        return;
    if (!CUSTOM_TEST_KERNEL::KERNEL_TEST(map.merge_idx(cuvoxmap::Idx2D{1, 2}), 1 * 1u + 2 * 10u))
        return;
